...
export default function AdminUsers() {
    ...
    const UserActionsMenu = ({ user }: { user: UserProfile }) => (
        <DropdownMenu>
        ...
        <div className="space-y-6">
                                ...
                                <TableRow key={user.id}>
                                    <TableCell>
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-gray-700 to-gray-900 flex items-center justify-center text-white text-xs font-bold overflow-hidden border border-border shrink-0">
                                                {user.avatar_url ? (
            ...
            <div className="md:hidden space-y-3">
                {loading ? (
                    ...
                    <div className="text-center text-muted-foreground py-12">Kullanıcı bulunamadı.</div>
                ) : (
                    filteredUsers.map((user) => (
                        <div key={user.id} className="bg-card border border-border rounded-xl p-4 space-y-3">
                            <div className="flex items-center justify-between gap-2">
                                <div className="flex items-center gap-3 min-w-0">
                                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gray-700 to-gray-900 flex items-center justify-center text-white text-sm font-bold overflow-hidden border border-border shrink-0">
                                        {user.avatar_url ? (
                                            <img src={user.avatar_url} className="w-full h-full object-cover" />
                                        ) : (
                                            user.username.charAt(0).toUpperCase()
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <p className="text-xs text-muted-foreground font-mono truncate">{user.id.slice(0, 16)}...</p>
                                    </div>
                                </div>
                                <UserActionsMenu user={user} />
                            </div>
                            ...
                            <p className="text-xs text-muted-foreground">
                                Katılım: {format(new Date(user.created_at), 'd MMM yyyy', { locale: tr })}
                            </p>
                        </div>
                    ))
                )}
